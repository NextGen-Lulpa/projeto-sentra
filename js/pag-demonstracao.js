...
// movimento do mouse
    mapContainer.addEventListener('mousemove', (e) => {
        ...
        // le cor do pixel sob o cursor
        const pixel = ctx.getImageData(x, y, 1, 1).data;
        const alpha = pixel[3];

        //se o pixel n for transparente, significa que o mouse está no mapa
        if (alpha > 10) {
            ...
            const lat = (-5 - (y / canvas.height) * 25).toFixed(2);
            const lng = (-70 + (x / canvas.width) * 35).toFixed(2);
